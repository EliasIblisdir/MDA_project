...
gini_by_country = df.groupby("country").apply(
    lambda group: np.round(gini(group.groupby("Industry")["ecContribution"].sum().values),2)
).reset_index()
...
industries_by_country = df.groupby(["country", "Industry"]).size().reset_index(name="count")
...
def choropleth_map_industries():
        ...
        fig = px.choropleth(
            ...
            range_color=(0, 1),
            title="Gini Coefficient of Funding across Sectors/Industries, by Country (Lower Gini coefficients are indicative of a more equal distribution of funding)"
...
def industry_pie(country):
    ...
    # Filter for selected country and drop missing Industry values
    data = df[(df["country"] == country) & (df["Industry"].notna())]

    if data.empty:
        return go.Figure().update_layout(title="No industry funding data available")

    # Aggregate total ecContribution per Industry
    data = data.groupby("Industry", as_index=False)["ecContribution"].sum()

    if data.empty:
        return go.Figure().update_layout(title="No industry funding data available")

    fig = px.pie(data, names="Industry", values="ecContribution", title=f"Funding apportioned by sector/industry in: {country_name}")
    return fig
...
app_ui = ui.page_fluid(
    ui.h2("EU Horizon Dashboard: Distribution of Funding across Sectors/Industries"),
    ui.input_select("country_select", "Select a Country Code (see map)", choices=sorted(df["country"].dropna().unique())),
    output_widget("map_industries"),
    ui.div(
        output_widget("sme"),
...
def server(input, output, session):
    ...
    @render_widget
    def industries():
        return industry_pie(input["country_select"]())
